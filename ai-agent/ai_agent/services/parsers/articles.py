...
def build_article_source_doc(
    ...
    applied_count = sum(1 for item in (ocr_blocks or []) if normalize_ocr_markdown(item))
    if image_count == 0:
    ...
    for item in ocr_blocks or []:
        markdown_block = normalize_ocr_markdown(item)
...
def normalize_ocr_markdown(item: dict[str, Any]) -> str | None:
    return normalize_text(item.get("markdown"))
