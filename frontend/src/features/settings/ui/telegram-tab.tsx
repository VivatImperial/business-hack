    ...
    return (
        <div className="flex flex-col gap-6 animate-in fade-in duration-200">
            {hasActiveSession && !changingUser ? (
                <>
                    {/* Settings list — clean card style */}
                    <div className="rounded-2xl border border-border/60 divide-y divide-border/60 overflow-hidden">
                        ...
                        <div className="flex items-center gap-4 px-5 py-4 bg-background">
                            ...
                            <div className="flex-1 min-w-0">
                                <p className={`text-[14px] font-semibold ${hasError ? "text-red-600" : "text-foreground"}`}>
                                    {hasError ? "Проблема с подключением" : "Telegram подключён"}
                                </p>
                                <p className={`text-[12px] mt-0.5 leading-relaxed ${hasError ? "text-red-500" : "text-muted-foreground"}`}>
                                    {hasError
                                        ? data.lastError
                                        : "Аккаунт привязан и готов к работе"}
                                </p>
                            </div>
                            <div
                                className={`size-2 rounded-full shrink-0 ${hasError ? "bg-red-500" : "bg-emerald-500"}`}
                            />
                        </div>
                        ...
                        <button
                            ...
                            className="flex items-center justify-between w-full px-5 py-4 bg-background text-left hover:bg-muted/40 transition-colors"
                        >
                            <p className="text-[14px] font-medium text-foreground">
                                Сменить аккаунт
                            </p>
